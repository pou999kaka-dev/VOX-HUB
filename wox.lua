...
-- HUB
	...
	local function createTPVisual(pos)
		...
		selection.AlwaysOnTop = true -- Ver através de paredes
		selection.Parent = tpMarker
		
		local glow = Instance.new("Part")
		glow.Shape = Enum.PartType.Cylinder
		glow.Size = Vector3.new(0.05, 4.8, 4.8)
		glow.CFrame = tpMarker.CFrame
		glow.Anchored = true
		glow.CanCollide = false
		glow.Transparency = 0.6
		glow.Material = Enum.Material.Neon
		glow.Color = Color3.fromRGB(0, 170, 255)
		glow.Parent = tpMarker
...
local function createHub()
	...
	local function animateClick(button)
		...
		local shrinkSize = UDim2.new(originalSize.X.Scale, originalSize.X.Offset * 0.9, originalSize.Y.Scale, originalSize.Y.Offset * 0.9)
		
		TweenService:Create(button, TweenInfo.new(0.05, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = shrinkSize}):Play()
