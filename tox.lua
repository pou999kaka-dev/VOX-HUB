...
-- helper functions for UI animations (shrink + ripple/wave)
local function darkerColor(c, factor)
    factor = factor or 0.8
    return Color3.new(math.clamp(c.R * factor, 0, 1), math.clamp(c.G * factor, 0, 1), math.clamp(c.B * factor, 0, 1))
end

local function playButtonClickAnim(button, waveColor)
    -- non-blocking; quick shrink and expand using UIScale
    local scale = button:FindFirstChild("VoxClickScale")
    if not scale then
        scale = Instance.new("UIScale")
        scale.Name = "VoxClickScale"
        scale.Parent = button
    end

    -- play shrink then back
    local shrink = TweenService:Create(scale, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 0.92})
    local expand = TweenService:Create(scale, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1})
    shrink:Play()
    shrink.Completed:Connect(function() expand:Play() end)

    -- ripple / wave effect: a rounded frame that grows and fades
    local wave = Instance.new("Frame")
    wave.Name = "VoxWave"
    wave.AnchorPoint = Vector2.new(0.5, 0.5)
    wave.Size = UDim2.new(0, 0, 0, 0)
    wave.Position = UDim2.new(0.5, 0, 0.5, 0)
    wave.BackgroundColor3 = darkerColor(waveColor or button.BackgroundColor3, 0.85)
    wave.BackgroundTransparency = 0.55
    wave.ZIndex = (button.ZIndex or 1) - 1 -- keep below text
    wave.Parent = button
    ...
    local waveTween = TweenService:Create(wave, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- HUB
	...
	local function createTPVisual(pos)
		...
		-- SelectionBox kept but hidden in favor of AlwaysOnTop adornment (so it's visible through walls)
		...
		-- Use BoxHandleAdornment to ensure the "quadrado" (square) is visible through walls.
		-- This adornment is AlwaysOnTop so you can see it even if there are objects between camera and marker.
		local ok, adornErr = pcall(function()
			local boxAdornment = Instance.new("BoxHandleAdornment")
			boxAdornment.Name = "VoxTPAdornment"
			boxAdornment.Adornee = tpMarker
			boxAdornment.Size = tpMarker.Size
			boxAdornment.AlwaysOnTop = true
			boxAdornment.ZIndex = 2
			boxAdornment.Color3 = Color3.fromRGB(0, 255, 255)
			boxAdornment.Transparency = 0.45
			boxAdornment.Parent = tpMarker
			tpAdornment = boxAdornment
		end)
		-- if BoxHandleAdornment fails (rare), we still have the SelectionBox albeit not AlwaysOnTop.
		...
		-- ROTATION: fixed speed, framerate-independent (neither too fast nor too slow)
		-- We rotate around the Y axis at 45 degrees per second (adjustable).
		local ANGLE_PER_SECOND = math.rad(45) -- 45 degrees/sec
		local conn
		conn = RunService.Heartbeat:Connect(function(dt)
			if not tpMarker or not tpMarker.Parent then
				if conn then conn:Disconnect() end
				return
			end
			-- rotate around Y (smooth, fixed speed)
			tpMarker.CFrame = tpMarker.CFrame * CFrame.Angles(0, ANGLE_PER_SECOND * dt, 0)
...
local function createHub()
	...
	resetBtn.MouseButton1Click:Connect(function()
		-- play click animation (shrink + wave) while also performing reset behavior
		playButtonClickAnim(resetBtn, resetBtn.BackgroundColor3)
		savedSpeed = defaultSpeed
		speedBox.Text = tostring(defaultSpeed)
		applySpeed(defaultSpeed)
		saveConfig()
		resetBtn.Rotation = 0
		-- keep the rotation/spin animation for reset (user asked to keep spin here)
	...
	jumpBtn.MouseButton1Click:Connect(function()
		playButtonClickAnim(jumpBtn, jumpBtn.BackgroundColor3)
	...
	noclipBtn.MouseButton1Click:Connect(function()
		playButtonClickAnim(noclipBtn, noclipBtn.BackgroundColor3)
	...
	espBtn.MouseButton1Click:Connect(function()
		playButtonClickAnim(espBtn, espBtn.BackgroundColor3)
	...
	tpBtn.MouseButton1Click:Connect(function()
		playButtonClickAnim(tpBtn, tpBtn.BackgroundColor3)
	...
	saveBtn.MouseButton1Click:Connect(function()
		-- remove the rotating spin that was on P (user requested no spin for P)
		playButtonClickAnim(saveBtn, saveBtn.BackgroundColor3)
		local char = player.Character
		if char and char:FindFirstChild("HumanoidRootPart") then
			savedPos = char.HumanoidRootPart.CFrame
					pcall(function() createTPVisual(savedPos) end)
					pcall(function() showTPSavedMessage() end)
			-- removed rotation spin here to satisfy the user's request
	...
	close.MouseButton1Click:Connect(function()
		playButtonClickAnim(close, close.BackgroundColor3)
