--// VOX SPEED HUB v2.0 + KEY SYSTEM (TELEPORT UPDATE)
...
local function createHub()
	...
	local finalSize = UDim2.new(0,230,0,300) -- Aumentado para caber o teleporte
	...
	local function createParticles(originX, originY, count, color, sizeMult)
		...
		local sMult = sizeMult or 1
		
		for i = 1, count do
			local particle = Instance.new("Frame")
			particle.Size = UDim2.new(0, math.random(3, 6) * sMult, 0, math.random(3, 6) * sMult)
			...
			local angle = math.random() * 2 * math.pi
			local distance = math.random(50, 120) * sMult
	...
	saveBtn.Size = UDim2.new(0,0,0,34) -- Começa invisível
	...
	tpBtn.MouseButton1Click:Connect(function()
		if tpBtn.Text == "SALVAR POSIÇÃO" then
			...
			if char and char:FindFirstChild("HumanoidRootPart") then
				...
				-- Animação de encolher e aparecer o P
				TweenService:Create(tpBtn, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Size = UDim2.new(0.6,0,0,34)}):Play()
				task.wait(0.1)
	...
	saveBtn.MouseButton1Click:Connect(function()
		...
		if char and char:FindFirstChild("HumanoidRootPart") then
			savedPos = char.HumanoidRootPart.CFrame
			-- Animação de Giro
			TweenService:Create(saveBtn, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Rotation = saveBtn.Rotation + 360}):Play()
			-- Partículas Azuis Menores
