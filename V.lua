...
-- Ajuste da rotação do grad para velocidade fixa e estável (não muito rápida, nem lenta)
local gradRotationSpeedDeg = 45 -- graus por segundo (ajustável)
local gradRotConn = nil
gradRotConn = RunService.Heartbeat:Connect(function(dt)
    if grad and grad.Parent and keyGui and keyGui.Parent then
        grad.Rotation = (grad.Rotation + gradRotationSpeedDeg * dt) % 360
    else
        if gradRotConn then gradRotConn:Disconnect() gradRotConn = nil end
    end
end)

-- Loop de Animação da Aura (Blinking only, rotação agora feita com RunService acima)
task.spawn(function()
    local blink = true
...
-- HUB
	...
	local function createTPVisual(pos)
		...
		-- Highlight para garantir que o marcador seja visível através de paredes
		local markerHighlight = Instance.new("Highlight")
		markerHighlight.Name = "VoxTPHighlight"
		markerHighlight.Adornee = tpMarker
		markerHighlight.FillTransparency = 1
		markerHighlight.OutlineTransparency = 0
		markerHighlight.OutlineColor = Color3.fromRGB(0, 255, 255)
		markerHighlight.Parent = tpMarker
		...
		-- Rotação com velocidade fixa (graus por segundo)
		local tpRotationSpeedDeg = 45 -- ajustar aqui se quiser mais devagar/rápido
		local rotConn
		rotConn = RunService.Heartbeat:Connect(function(dt)
			if tpMarker and tpMarker.Parent then
				-- girar em torno do eixo Y (mais natural)
				tpMarker.CFrame = tpMarker.CFrame * CFrame.Angles(0, math.rad(tpRotationSpeedDeg * dt), 0)
			else
				if rotConn then rotConn:Disconnect() rotConn = nil end
			end
		end)
		
		-- quando o marcador for destruído, desconecta a rotação / highlight
		tpMarker.Destroying:Connect(function()
			if rotConn then rotConn:Disconnect() rotConn = nil end
		end)
...
local function createHub()
	...
	-- Helper: animação de clique (shrink + ripple). Não bloqueante.
	local function playClickAnim(button, rippleColor, keepRotation)
		task.spawn(function()
			if not button or not button.Parent then return end
			-- guarda propriedades originais
			local origSize = button.Size
			local origPos = button.Position
			-- calcula tamanho reduzido (pequena redução em pixels)
			local shrinkFactor = 0.92
			local newXOffset = math.floor(origSize.X.Offset * shrinkFactor)
			local newYOffset = math.floor(origSize.Y.Offset * shrinkFactor)
			-- se os offsets forem zero (tamanho em scale), tenta usar uma redução relativa
			if origSize.X.Offset == 0 then newXOffset = 0 end
			if origSize.Y.Offset == 0 then newYOffset = 0 end
			local newSize = UDim2.new(origSize.X.Scale, newXOffset, origSize.Y.Scale, newYOffset)
			local dx = (origSize.X.Offset - newSize.X.Offset) / 2
			local dy = (origSize.Y.Offset - newSize.Y.Offset) / 2
			local newPos = UDim2.new(origPos.X.Scale, origPos.X.Offset + dx, origPos.Y.Scale, origPos.Y.Offset + dy)

			-- Shrink tween
			local t1 = TweenService:Create(button, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = newSize, Position = newPos})
			local t2 = TweenService:Create(button, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = origSize, Position = origPos})
			t1:Play()

			-- Ripple
			local ripple = Instance.new("Frame")
			ripple.Name = "VoxRipple"
			ripple.AnchorPoint = Vector2.new(0.5, 0.5)
			ripple.Size = UDim2.new(0, 0, 0, 0)
			ripple.Position = UDim2.new(0.5, 0, 0.5, 0)
			ripple.BackgroundColor3 = rippleColor or Color3.fromRGB(60,60,60)
			ripple.BackgroundTransparency = 0
			ripple.BorderSizePixel = 0
			ripple.ZIndex = (button.ZIndex and math.max(0, button.ZIndex - 1)) or 0
			ripple.Parent = button
			local rCorner = Instance.new("UICorner")
			rCorner.CornerRadius = UDim.new(1, 0)
			rCorner.Parent = ripple

			-- calcula target size (um pouco maior que o botão)
			local targetW = math.max(origSize.X.Offset * 2.2, 150)
			local targetH = math.max(origSize.Y.Offset * 2.2, 150)
			local rippleTween = TweenService:Create(ripple, TweenInfo.new(0.35, Enum.EasingStyle.Quad), {
				Size = UDim2.new(0, targetW, 0, targetH),
				BackgroundTransparency = 1,
			})
			-- espera shrink terminar para crescer ripple e voltar
			...
			-- garante que o botão volte ao normal se algo não conectar
			t2.Completed:Connect(function() end)
		end)
	end
	...
	igBtn.MouseButton1Click:Connect(function()
		...
		playClickAnim(igBtn, Color3.fromRGB(0,90,190), false)
	...
	resetBtn.MouseButton1Click:Connect(function()
		-- animação de clique: shrink + ripple + manter rotação atual (giro continua)
		playClickAnim(resetBtn, Color3.fromRGB(70,70,70), true)
	...
	jumpBtn.MouseButton1Click:Connect(function()
		-- animação de clique
		playClickAnim(jumpBtn, Color3.fromRGB(50,50,50), false)
	...
	noclipBtn.MouseButton1Click:Connect(function()
		-- animação de clique
		playClickAnim(noclipBtn, Color3.fromRGB(50,50,50), false)
	...
	espBtn.MouseButton1Click:Connect(function()
		-- animação de clique
		playClickAnim(espBtn, Color3.fromRGB(50,50,50), false)
	...
	tpBtn.MouseButton1Click:Connect(function()
		-- animação de clique
		playClickAnim(tpBtn, Color3.fromRGB(20,60,120), false)
	...
	saveBtn.MouseButton1Click:Connect(function()
		-- animação de clique (remove animação de giro do P)
		playClickAnim(saveBtn, Color3.fromRGB(10,60,130), false)
		...
		if char and char:FindFirstChild("HumanoidRootPart") then
			savedPos = char.HumanoidRootPart.CFrame
					pcall(function() createTPVisual(savedPos) end)
					pcall(function() showTPSavedMessage() end)
			-- removei a animação de giro do botão P conforme pedido
	...
	close.MouseButton1Click:Connect(function()
		-- animação de clique no X
		playClickAnim(close, Color3.fromRGB(40,40,40), false)
