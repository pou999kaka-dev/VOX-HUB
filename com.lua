...
-- Modified to:
-- - Make teleport marker rotate at a stable "normal" speed (frame-rate independent).
-- - Show the "distância" label inside a rounded rectangle with 50% transparency.
-- - Make teleport marker and distance label visible through walls (AlwaysOnTop UI).
-- - Add click press + wave animations to all buttons in Key System and Hub.
-- - Preserve and combine existing rotation animations (Reset / P) with new click effects.
-- - Add a teleport visual animation at source and destination when teleporting.
...
-- Utility: scale a UDim2 by factor (keeps numeric offsets rounded)
local function scaleUDim2(u, factor)
    return UDim2.new(u.X.Scale * factor, math.floor(u.X.Offset * factor), u.Y.Scale * factor, math.floor(u.Y.Offset * factor))
end

-- Utility: create button press + wave animations and hook them to a button
local function addButtonEffects(button)
    if not button or not button:IsA("GuiButton") then return end
    button.AutoButtonColor = false

    local originalSize = button.Size
    local pressedSize = scaleUDim2(originalSize, 0.96)

    local function playPressAnim()
        -- quick shrink
        local ok, _ = pcall(function()
            TweenService:Create(button, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = pressedSize}):Play()
        end)
        -- return to normal quickly
        task.delay(0.09, function()
            pcall(function()
                TweenService:Create(button, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = originalSize}):Play()
            end)
        end)
    end
    ...
    local function playWave()
        ...
        wave.Name = "VoxWave"
        wave.AnchorPoint = Vector2.new(0, 0)
        wave.Position = UDim2.new(0, 0, 0, 0)
        wave.Size = UDim2.new(0, 0, 1, 0) -- start with 0 width
        wave.BackgroundTransparency = 0.6
        -- darker color based on button background
        local bcol = button.BackgroundColor3 or Color3.fromRGB(30,30,30)
        local dark = Color3.new(math.clamp(bcol.R * 0.75, 0, 1), math.clamp(bcol.G * 0.75, 0, 1), math.clamp(bcol.B * 0.75, 0, 1))
        wave.BackgroundColor3 = dark
        wave.ZIndex = (button:IsA("GuiObject") and button.ZIndex + 1) or 2
        wave.Parent = button
        ...
        -- animate width to full and fade out
        TweenService:Create(wave, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1}):Play()
        Debris:AddItem(wave, 0.6)
    end

    -- Hook both MouseButton1Down and MouseButton1Click so effect is immediate and returns
...
-- Generic wave/press can be used in both Key GUI and Hub GUI. We'll call addButtonEffects on each button created.
...
-- attach click effects to Key System buttons
addButtonEffects(confirm)
addButtonEffects(getLink)
...
-- HUB
	...
	local tpRotationConnection = nil
	...
	local function createTPVisual(pos)
		...
		if tpRotationConnection then
			pcall(function() tpRotationConnection:Disconnect() end)
			tpRotationConnection = nil
		end
		
		-- MARKER PART (minimal/used as adornee). We'll keep it simple and use a small anchored part.
		tpMarker = Instance.new("Part")
		tpMarker.Name = "VoxTPMarker"
		tpMarker.Size = Vector3.new(2, 0.2, 2) -- thin square plate
		tpMarker.CFrame = pos
		tpMarker.Anchored = true
		tpMarker.CanCollide = false
		tpMarker.Transparency = 1 -- make the part invisible; visuals handled by BillboardGui (AlwaysOnTop)
		tpMarker.Parent = workspace
		
		-- BillboardGui for the square marker (AlwaysOnTop so it shows through walls)
		tpBillboard = Instance.new("BillboardGui")
		tpBillboard.Name = "VoxTPSquare"
		tpBillboard.Adornee = tpMarker
		tpBillboard.Size = UDim2.new(0, 120, 0, 120)
		tpBillboard.StudsOffset = Vector3.new(0, 1.5, 0)
		...
		square.Position = UDim2.new(0.5, -40, 0.5, -40)
		square.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
		square.BackgroundTransparency = 0.2
		square.Parent = tpBillboard
		local sqCorner = Instance.new("UICorner")
		sqCorner.CornerRadius = UDim.new(0, 10)
		sqCorner.Parent = square
		
		-- Distance label container (rounded rectangle with 50% transparency)
		local distContainer = Instance.new("Frame")
		distContainer.Name = "DistContainer"
		distContainer.Size = UDim2.new(0, 140, 0, 36)
		distContainer.Position = UDim2.new(0.5, -70, 1, -10)
		distContainer.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
		distContainer.BackgroundTransparency = 0.5 -- 50% transparency
		distContainer.Parent = tpBillboard
		distContainer.AnchorPoint = Vector2.new(0.5, 1)
		local dcCorner = Instance.new("UICorner")
		dcCorner.CornerRadius = UDim.new(0, 10)
		dcCorner.Parent = distContainer
		
		local distLabel = Instance.new("TextLabel")
		distLabel.Name = "DistLabel"
		distLabel.Size = UDim2.new(1, -8, 1, -6)
		distLabel.Position = UDim2.new(0, 4, 0, 3)
		distLabel.BackgroundTransparency = 1
		distLabel.Font = Enum.Font.GothamBold
		distLabel.TextColor3 = Color3.fromRGB(200, 255, 255)
		distLabel.TextSize = 16
		distLabel.TextStrokeTransparency = 0
		distLabel.Text = "distância: 0"
		distLabel.TextXAlignment = Enum.TextXAlignment.Center
		distLabel.Parent = distContainer
		
		-- Stable rotation (frame-rate independent). We'll rotate the billboard's adornee part around Y axis slowly.
		-- Use RunService.Heartbeat so speed stays normal regardless of other script activity.
		local rotationSpeedDegPerSec = 30 -- moderate speed (30 degrees per second)
		tpRotationConnection = RunService.Heartbeat:Connect(function(dt)
			if tpMarker and tpMarker.Parent then
				-- rotate around Y axis
				local cf = tpMarker.CFrame
				local rot = CFrame.Angles(0, math.rad(rotationSpeedDegPerSec * dt), 0)
				tpMarker.CFrame = cf * rot
			end
		end)
		
		-- Update distance text loop
		task.spawn(function()
			while tpMarker and tpBillboard and tpMarker.Parent do
				local char = player.Character
				if char and char:FindFirstChild("HumanoidRootPart") then
					local d = (char.HumanoidRootPart.Position - tpMarker.Position).Magnitude
					local lbl = tpBillboard:FindFirstChild("DistContainer") and tpBillboard.DistContainer:FindFirstChild("DistLabel")
					if lbl then lbl.Text = "distância: " .. math.floor(d) end
				end
				task.wait(0.2)
			end
		end)
...
local function createHub()
	...
	-- add effects to top bar close button
	addButtonEffects(close)
	...
	-- Add our press + wave effects to reset button in addition to its rotation animation
	...
	-- add press + wave effects to tp buttons (and preserve rotation animation on saveBtn)
	addButtonEffects(tpBtn)
	addButtonEffects(saveBtn)

	local function playTeleportEffectAt(position, color)
		-- small ring expands and fades
		local ring = Instance.new("Part")
		ring.Size = Vector3.new(1, 0.2, 1)
		ring.CFrame = CFrame.new(position.Position) * CFrame.Angles(math.rad(90), 0, 0)
		ring.Anchored = true
		ring.CanCollide = false
		ring.Material = Enum.Material.Neon
		ring.Color = color or BLUE_COLOR
		ring.Parent = workspace
		ring.Transparency = 0
		Debris:AddItem(ring, 0.8)
		-- tween size and transparency
		local tween = TweenService:Create(ring, TweenInfo.new(0.55, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = Vector3.new(12, 0.2, 12), Transparency = 1})
		tween:Play()
	end
	...
	tpBtn.MouseButton1Click:Connect(function()
		...
		elseif tpBtn.Text == "TELEPORTE" and savedPos then
			local char = player.Character
			if char and char:FindFirstChild("HumanoidRootPart") then
				-- teleport animation: effect at origin, then move, then effect at destination
				local hrp = char.HumanoidRootPart
				local originCFrame = hrp.CFrame
				-- play origin effect
				pcall(function() playTeleportEffectAt(originCFrame, BLUE_COLOR) end)
				-- small delay to feel the teleport (so user sees effect), then teleport
				task.wait(0.12)
				hrp.CFrame = savedPos
				-- destination effect
				pcall(function() playTeleportEffectAt(savedPos, BLUE_COLOR) end)
				-- optional small particle
