--// VOX SPEED HUB v2.0 + KEY SYSTEM (FINAL DEFINITIVO - RGB & PARTICLES)
...
-- Função para obter cor RGB baseada no tempo
...
box.Size = UDim2.new(0,320,0,240) -- Aumentado para caber o botão de copiar
...
-- Partículas RGB para o Key System
task.spawn(function()
    while keyGui.Parent do
        if box.BackgroundTransparency < 0.5 then
            local p = Instance.new("Frame")
            p.Size = UDim2.new(0, math.random(2, 5), 0, math.random(2, 5))
            p.BackgroundColor3 = isCorrect == true and GREEN_COLOR or (isCorrect == false and RED_COLOR or getRGB())
            p.BorderSizePixel = 0
            ...
            local targetPos = p.Position + UDim2.new(0, math.cos(angle) * dist, 0, math.sin(angle) * dist)
            
            TweenService:Create(p, TweenInfo.new(1), {Position = targetPos, BackgroundTransparency = 1}):Play()
...
confirm.Position = UDim2.new(0.1,0,0.55,0)
...
local copyKeyBtn = Instance.new("TextButton", box)
copyKeyBtn.Size = UDim2.new(0.8,0,0,36)
copyKeyBtn.Position = UDim2.new(0.1,0,0.75,0)
copyKeyBtn.Text = "COPIAR KEY"
copyKeyBtn.Font = Enum.Font.GothamBold
copyKeyBtn.TextSize = 15
copyKeyBtn.TextColor3 = WHITE_COLOR
copyKeyBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
copyKeyBtn.BorderSizePixel = 0
copyKeyBtn.TextTransparency = 1
Instance.new("UICorner", copyKeyBtn).CornerRadius = UDim.new(0,12)
...
local function showKeyPrompt()
    ...
    TweenService:Create(copyKeyBtn,TweenInfo.new(0.5),{TextTransparency=0}):Play()
end
...
copyKeyBtn.MouseButton1Click:Connect(function()
    setclipboard("VOX-2025")
    copyKeyBtn.Text = "COPIADO!"
    task.wait(1)
    copyKeyBtn.Text = "COPIAR KEY"
end)
...
local function createHub()
	...
	local function createParticles(originX, originY, count, color)
		...
		for i = 1, count do
			...
			particle.BackgroundColor3 = color or getRGB()
	...
	-- Loop de partículas constantes ao redor do Hub
	task.spawn(function()
		while main.Parent do
			local p = Instance.new("Frame")
			p.Size = UDim2.new(0, math.random(3, 6), 0, math.random(3, 6))
			p.BackgroundColor3 = getRGB()
			p.BorderSizePixel = 0
			p.Position = UDim2.new(math.random(), 0, math.random(), 0)
			p.Parent = main
			Instance.new("UICorner", p).CornerRadius = UDim.new(1, 0)
			...
			TweenService:Create(p, TweenInfo.new(1.2), {Position = targetPos, BackgroundTransparency = 1}):Play()
	...
	version.Size = UDim2.new(0.5,0,0,20)
	version.Position = UDim2.new(0,10,1,-25)
	version.BackgroundTransparency = 1
	version.Text = "v2.0 | @paopremium15"
	...
	copyBtn.Position = UDim2.new(1,-90,1,-25)
	copyBtn.BackgroundTransparency = 1
	copyBtn.Text = "Copiar Insta"
	...
	local function createESP(p)
		...
		local function updateESP()
			...
			if char and char:FindFirstChild("HumanoidRootPart") then
				local highlight = char:FindFirstChild("VoxESP") or Instance.new("Highlight", char)
		...
		if p.Character then updateESP() end
	end
	...
	RunService.RenderStepped:Connect(function()
		for _, p in pairs(Players:GetPlayers()) do
			if p.Character and p.Character:FindFirstChild("VoxESP") then
				p.Character.VoxESP.Enabled = espEnabled
			end
		end
	end)
	...
	topBar.InputBegan:Connect(function(i)
		if valid(i) and not minimized then
			dragging = true
			dragStart = i.Position
			startPos = main.Position
			TweenService:Create(hubAura,TweenInfo.new(0.2),{Thickness=6}):Play()
		end
	end)
	...
	UIS.InputChanged:Connect(function(i)
		if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
			...
			-- Partículas ao arrastar
			if math.random(1, 5) == 1 then
				local pX, pY = main.AbsolutePosition.X + main.AbsoluteSize.X/2, main.AbsolutePosition.Y + main.AbsoluteSize.Y/2
				for i=1, 3 do
					local particle = Instance.new("Frame", gui)
					particle.Size = UDim2.new(0, 4, 0, 4)
					particle.BackgroundColor3 = getRGB()
...
confirm.MouseButton1Click:Connect(function()
    if validKeys[keyBox.Text] then
        ...
        shake()
        task.wait(0.5)
        isCorrect = nil -- Volta para o RGB
    end
end)
