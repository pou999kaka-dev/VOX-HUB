...
local function createHub()
	...
	copiedMsg.Size = UDim2.new(0, 250, 0, 40)
	copiedMsg.Position = UDim2.new(0.5, -125, 0.5, -20)
	copiedMsg.BackgroundTransparency = 1
	copiedMsg.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
	copiedMsg.Text = "link do Instagram copiado!"
	copiedMsg.Font = Enum.Font.GothamBold
	copiedMsg.TextSize = 16
	...
	local function showCopiedAnimation()
		...
		copiedMsg.Position = UDim2.new(0.5, -125, 0.5, 20)
		TweenService:Create(copiedMsg, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {TextTransparency = 0, BackgroundTransparency = 0.2, Position = UDim2.new(0.5, -125, 0.5, -20)}):Play()
		task.wait(1.5)
		TweenService:Create(copiedMsg, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {TextTransparency = 1, BackgroundTransparency = 1, Position = UDim2.new(0.5, -125, 0.5, -60)}):Play()
		task.wait(0.4)
		copiedMsg.Visible = false
	end
	...
	title.Position = UDim2.new(0,45,0,0) -- Ajustado para dar espaço ao novo botão
	title.BackgroundTransparency = 1
	title.Text = "VOX SPEED HUB"
	...
	-- NOVO BOTÃO DO INSTAGRAM (BOLA AZUL)
	...
	igBtn.MouseButton1Click:Connect(function()
		...
		TweenService:Create(igBtn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 25, 0, 25), Position = UDim2.new(0, 12.5, 0.5, -12.5)}):Play()
		task.wait(0.1)
		TweenService:Create(igBtn, TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = originalSize, Position = originalPos}):Play()
		
		setclipboard(INSTAGRAM_LINK)
		task.spawn(showCopiedAnimation)
	end)

	-- SPEED CONTROLS
	local speedLabel = Instance.new("TextLabel")
	speedLabel.Size = UDim2.new(0.8,0,0,20)
	speedLabel.Position = UDim2.new(0.1,0,0,55)
	speedLabel.BackgroundTransparency = 1
	speedLabel.Text = "VELOCIDADE"
	speedLabel.Font = Enum.Font.GothamBold
	speedLabel.TextSize = 14
	speedLabel.TextColor3 = WHITE_COLOR
	speedLabel.Parent = main
	
	local speedBox = Instance.new("TextBox")
	speedBox.Size = UDim2.new(0.55,0,0,34)
	...
	resetBtn.Position = UDim2.new(0.7,0,0,80)
	...
	resetBtn.MouseButton1Click:Connect(function()
		...
		-- Reset Rotation before animation to prevent upside down
		resetBtn.Rotation = 0
	...
	-- TELEPORTE SYSTEM (FIXED LAYOUT & PARTICLES)
	...
	tpBtn.MouseButton1Click:Connect(function()
		if tpBtn.Text == "SALVAR POSIÇÃO" then
			...
			if char and char:FindFirstChild("HumanoidRootPart") then
				...
				-- Animação de encolher e aparecer o P (sem espaço entre eles)
	...
	close.MouseButton1Click:Connect(function()
		...
		-- Animação de fechar (encolher de baixo para cima)
		-- Para fazer de baixo para cima, mudamos o AnchorPoint temporariamente ou calculamos a posição
		...
		TweenService:Create(main, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = miniSize}):Play()
	end)
	
	topBar.InputEnded:Connect(function(i)
		if minimized and (i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch) then
			minimized = false
			TweenService:Create(main, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = fullSize}):Play()
			task.wait(0.3)
			for _, v in ipairs(main:GetChildren()) do
				if v:IsA("GuiObject") and v ~= topBar then 
					if v == saveBtn and tpBtn.Text ~= "TELEPORTE" then
						v.Visible = false
					else
						v.Visible = true 
					end
