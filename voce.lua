...
-- Utilitários de UI: animação de botão (press + ripple)
local function darkenColor(col, factor)
    factor = factor or 0.85
    return Color3.new(math.clamp(col.R * factor, 0, 1), math.clamp(col.G * factor, 0, 1), math.clamp(col.B * factor, 0, 1))
end

-- Aplica efeitos a um TextButton (pressionar + onda)
local function attachButtonEffects(btn, opts)
    opts = opts or {}
    local waveColor = opts.waveColor or darkenColor(btn.BackgroundColor3, 0.85)
    -- ensure there is a UIScale
    ...
    local function playScale()
        ...
        local shrink = TweenService:Create(uiScale, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 0.92})
        local expand = TweenService:Create(uiScale, TweenInfo.new(0.14, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1})
        shrink:Play()
    ...
    local function playWave()
        -- get absolute size (fallback if not ready)
        local absX, absY = btn.AbsoluteSize.X, btn.AbsoluteSize.Y
        if absX == 0 or absY == 0 then
            -- wait a frame to get sizes
            task.wait()
            absX, absY = btn.AbsoluteSize.X, btn.AbsoluteSize.Y
        end
        local maxDim = math.max(absX, absY)
        local startSize = 8
        local finalSize = maxDim * 2

        local wave = Instance.new("Frame")
        wave.Name = "VoxWave"
        wave.AnchorPoint = Vector2.new(0.5, 0.5)
        wave.Position = UDim2.new(0.5, 0, 0.5, 0)
        wave.Size = UDim2.new(0, startSize, 0, startSize)
        wave.BackgroundColor3 = waveColor
        wave.BorderSizePixel = 0
        wave.ZIndex = btn.ZIndex + 5
        wave.Visible = true
        wave.Parent = btn
        wave.ClipsDescendants = false
        local corner = Instance.new("UICorner")
        corner.CornerRadius = UDim.new(1, 0)
        corner.Parent = wave

        -- make sure it doesn't capture input (TextButton will still receive events)
        pcall(function() wave.Active = false end)

        local t1 = TweenService:Create(wave, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, finalSize, 0, finalSize),
            BackgroundTransparency = 1
        })
        local posTween = TweenService:Create(wave, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    -- Play on down so it doesn't block functionality
...
-- Attach effects to key system buttons
attachButtonEffects(confirm)
attachButtonEffects(getLink)
...
-- HUB
    ...
    local tpRotConn = nil
    local TP_ROTATION_SPEED = 1.2 -- radians per second, velocidade fixa e moderada
	
	local function createTPVisual(pos)
		if tpMarker then 
            pcall(function() tpMarker:Destroy() end)
            if tpRotConn then
                pcall(function() tpRotConn:Disconnect() end)
                tpRotConn = nil
            end
        end
		...
		-- Highlight para enxergar através das paredes
		local h = Instance.new("Highlight")
		h.Name = "VoxTPHighlight"
		h.Adornee = tpMarker
		...
		-- Rotação com velocidade fixa independente do FPS
        tpRotConn = RunService.Heartbeat:Connect(function(dt)
            if tpMarker and tpMarker.Parent then
                tpMarker.CFrame = tpMarker.CFrame * CFrame.Angles(TP_ROTATION_SPEED * dt, 0, 0)
            end
            -- atualiza distância aqui também
...
local function createHub()
	...
	-- Attach button effects
	attachButtonEffects(close)
	attachButtonEffects(igBtn)
	...
	-- Attach effects (resetBtn keeps its spin animation too)
	attachButtonEffects(resetBtn)
	...
	-- Attach effects (saveBtn will still keep its rotation animation)
	attachButtonEffects(tpBtn)
	attachButtonEffects(saveBtn)
