...
-- Lógica de Partículas para o Key System (Igual ao Hub Principal)
local function spawnKeyParticles()
    ...
    for k = 1, 8 do
        ...
        particle.BackgroundColor3 = stroke.Color -- Segue a cor da aura (stroke)
...
-- Loop de Animação da Aura e Partículas do Key System
task.spawn(function()
    local colorToggle = true
    while keyGui.Parent do
        grad.Rotation += 2
        
        -- Animação de cor da aura (Igual ao Hub Principal)
        local targetColor = colorToggle and WHITE_COLOR or Color3.new(0,0,0)
        TweenService:Create(stroke, TweenInfo.new(2.5), {Color = targetColor}):Play()
        
        -- Spawn contínuo de partículas
        spawnKeyParticles()
...
-- Loop secundário para partículas mais frequentes
task.spawn(function()
    while keyGui.Parent do
        spawnKeyParticles()
        task.wait(0.1)
