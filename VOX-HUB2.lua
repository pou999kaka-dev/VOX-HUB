...
task.spawn(function()
    ...
    while keyGui.Parent do
        -- Rotação do Gradiente (Faz a volta no Hub)
        grad.Rotation += 4 -- Velocidade equilibrada
        
        -- Animação do ":" piscando
        blinkCounter += 1
        if blinkCounter >= 15 then -- Controla a velocidade do piscar
            blink = not blink
            keyBox.PlaceholderText = blink and "Key:" or "Key "
            blinkCounter = 0
        end
        
        task.wait(0.03) -- Atualização suave da rotação
    end
end)
